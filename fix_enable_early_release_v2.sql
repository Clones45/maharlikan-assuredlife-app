...
BEGIN
    ...
    -- FORCE make valid commissions receivable if they arent already
    -- (Safety check for Instant Release Mode)
    UPDATE commissions c
    SET is_receivable = true
    FROM agent_commission_rollups ar
    WHERE c.agent_id = ar.agent_id 
      AND c.period_year = ar.period_year 
      AND c.period_month = ar.period_month
      AND ar.agent_id IN (2, 5)
      AND ar.period_year = 2026 AND ar.period_month = 2;
    ...
    -- FORCE make valid commissions receivable for Agent 6 Jan
    UPDATE commissions c
    SET is_receivable = true
    FROM agent_commission_rollups ar
    WHERE c.agent_id = ar.agent_id 
      ...
      AND c.period_month = ar.period_month
      AND ar.agent_id = 6
      AND ar.period_year = 2026 AND ar.period_month = 1;

    -- 3. Recalculate Wallets
    FOR r IN 
